...
def fig20_profile_similarity(results):
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / 'fig20_profile_similarity.png', bbox_inches='tight')
    plt.close()
...
def fig21_domain_overlap_snr(results):
    ...
    for idx, model in enumerate(MODELS):
        ...
        ax.legend(handles=[above, below], loc='upper right', fontsize=8)
...
def fig22_architecture_vs_domain_summary(results, per_circuit):
    ...
    ax.text(0.5, 0.95, f'Mann-Whitney p < 0.000001',
    ...
    fig.suptitle('Architecture Over Knowledge: Energy Profile Comparison\n'
                 ...
                 fontsize=14, y=1.01)
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / 'fig22_architecture_vs_domain_summary.png', bbox_inches='tight')
    plt.close()
...
def fig23_evidence_accumulation_summary(results, per_circuit):
    ...
    for model in MODELS:
        ...
        if bn_corr:
            ...
            # Position label
            y_pos = 0.95 if model == 'gemma-2-2b' else 0.85
            ax.text(0.02, y_pos,

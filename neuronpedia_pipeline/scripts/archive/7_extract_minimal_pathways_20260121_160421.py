...
class MinimalPathwayExtractor:
    ...
    def _build_graph(self):
        ...
        # We'll need to load the converted file
        converted_path = self.analysis_path.parent / self.analysis_path.name.replace('_analysis.json', '_converted.json')
...
def main():
    ...
    parser.add_argument('--output-dir', type=str, required=True,
                       help='Output directory for minimal pathway results')
    ...
    extractor = MinimalPathwayExtractor(args.analysis)

    pathway_path = output_dir / "minimal_pathway.json"
    minimal_G, metrics = extractor.extract_minimal_pathway(pathway_path)

    if minimal_G:
        # Create visualization
        viz_path = output_dir / "minimal_pathway_comparison.png"
        extractor.visualize_comparison(minimal_G, metrics, viz_path)

        print("\n" + "="*60)
        print("MINIMAL PATHWAY EXTRACTION COMPLETE")
        print("="*60)
        print(f"Results: {pathway_path}")

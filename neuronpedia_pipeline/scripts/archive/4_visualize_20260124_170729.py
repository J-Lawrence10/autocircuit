...
for sn_id in SG.nodes():
    ...
    label = f"SN{sn_id}: {theme}\n"

    # Add feature descriptions if available
    if top_features:
        for _, _, desc in top_features[:2]:  # Top 2 features
            # Extract clean tokens from Neuronpedia description
            if "Activates on:" in desc:
                # Format: "Activates on: token1, token2, token3"
                tokens_part = desc.split("Activates on:")[1].strip()
                # Get first 2 tokens, clean them up
            ...
            # Escape matplotlib special characters to prevent math mode interpretation
            short_desc = short_desc.replace('$', r'\$').replace('{', r'\{').replace('}', r'\}').replace('_', r'\_').replace('^', r'\^')
    ...
    if max_layer >= 21:
        ...
        if target_token:
            ...
            safe_token = target_token.replace('$', r'\$').replace('{', r'\{').replace('}', r'\}').replace('_', r'\_').replace('^', r'\^')
...
# Legend - dynamically generate based on actual supernodes with themes
legend_elements = []
...
if output_supernodes:
    action_text += f"   • SN{output_supernodes[0]} (layers {min(supernode_stats[output_supernodes[0]]['layers'])}-{max(supernode_stats[output_supernodes[0]]['layers'])}) directs final output\n"
    action_text += f"   • Test ablation to verify necessity\n\n"
else:
    action_text += f"   • No output-layer supernodes detected\n\n"

action_text += f"2. Test Hub Supernodes:\n"
for sn_id in most_connected[:2]:
    action_text += f"   • SN{sn_id} ({SG.degree(sn_id)} connections) is a hub\n"
action_text += f"   • Likely critical for information flow\n\n"

action_text += f"3. Trace Information Flow:\n"
action_text += f"   • Follow edges from input (L0-5) to output\n"
action_text += f"   • Identify bottlenecks and parallel paths"

# Place action items BELOW the plot area
fig.text(0.5, -0.05, action_text, ha='center', fontsize=8,

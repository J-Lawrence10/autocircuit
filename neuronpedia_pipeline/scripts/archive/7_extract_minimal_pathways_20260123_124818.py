...
3. Identify Redundancy:
   ...
   • Investigate: Why so many parallel paths?
    ...
    def visualize_minimal_graph_structure(self, minimal_G, output_path, metrics):
        ...
        # 6. Add labels for top 20 most important nodes (by betweenness)
        ...
        for node in top_nodes:
            layer = minimal_G.nodes[node]['layer']
            # Extract feature ID from node ID (format: layer_feature_ctx)
            parts = node.split('_')
            if len(parts) >= 2:
                feature_id = parts[1]
                labels[node] = f"L{layer}_F{feature_id[:4]}"
        ...
        plt.tight_layout()
        plt.subplots_adjust(bottom=0.12)  # Make room for action items

...
def fig37_r2_cascade(d1):
    ...
    for idx, model in enumerate(['gemma-2-2b', 'qwen3-4b']):
        ax = axes[idx]
        reg = d1.get(model, {}).get('interaction_regression', {})
        if not reg:
        ...
        values = [reg.get('R2_additive', 0), reg.get('R2_with_category', 0),
                  reg.get('R2_with_energy_interaction', 0),
                  reg.get('R2_with_full_interactions', 0),
                  reg.get('R2_expanded', 0)]
...
def fig38_unused_correlations(d1):
    ...
    for idx, model in enumerate(['gemma-2-2b', 'qwen3-4b']):
        ax = axes[idx]
        corrs = d1.get(model, {}).get('unused_correlations', {})
        if not corrs:
            ax.text(0.5, 0.5, 'No data', ha='center', va='center')
            continue

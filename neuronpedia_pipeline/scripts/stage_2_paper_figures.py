...
def fig1_pipeline():
    ...
    fig.savefig(os.path.join(FIG_DIR, 'fig1_pipeline.png'))
    plt.close(fig)
...
def fig2_jaccard_heatmaps():
    ...
    fig.savefig(os.path.join(FIG_DIR, 'fig2_jaccard_heatmaps.png'))
    plt.close(fig)
...
def fig4_confidence_energy():
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(FIG_DIR, 'fig4_confidence_energy.png'))
    plt.close(fig)
...
def fig10_output_path():
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(FIG_DIR, 'fig10_output_path.png'))
    plt.close(fig)
...
def fig12_cohens_d():
    ...
    if im is not None:
        fig.colorbar(im, ax=axes, shrink=0.6, label="Cohen's d")
    fig.tight_layout(rect=[0, 0, 0.92, 0.95])
    fig.savefig(os.path.join(FIG_DIR, 'fig12_cohens_d.png'))
    plt.close(fig)
...
def fig13_activation_profiles():
    ...
    for mi, model in enumerate(MODELS):
        ...
        peak_data = model_data.get('peak_layer', {})
        ...
        for ci, cat in enumerate(CATEGORIES):
            ...
            if cat_peaks:
                ...
                ax1.scatter(np.ones(len(cat_peaks)) * ci + jitter, cat_peaks,
                          color=CATEGORY_COLORS[cat], s=60, alpha=0.7,
                          edgecolors='black', linewidth=0.5, label=CATEGORY_LABELS[cat])
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(FIG_DIR, 'fig13_activation_profiles.png'))
    plt.close(fig)

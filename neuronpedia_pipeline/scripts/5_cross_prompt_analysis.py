...
def main():
    ...
    parser.add_argument('--analyses', nargs='+',
                       help='Paths to analysis JSON files (if not provided, finds all available)')
    parser.add_argument('--output-name', type=str,
                       help='Name for this cross-analysis (e.g., "geography-domain")')
    ...
    # Auto-discover analysis files if not provided
    if args.analyses:
        analysis_files = [Path(p) for p in args.analyses]
    else:
        print("[INFO] Auto-discovering analysis files...")
    ...
    if len(analysis_files) < 2:
        print("[ERROR] Need at least 2 analysis files for cross-prompt comparison")
        print(f"Found: {len(analysis_files)}")
        return
    ...
    if args.output_name:
        ...
        # Auto-generate name from number of prompts
        output_name = f"cross-analysis-{len(analysis_files)}prompts"

    # Create output directory using PathManager
    output_dir = pm.get_cross_analysis_dir(output_name)
    print(f"\nOutput directory: {output_dir}")

    # Run analysis
    analyzer = CrossPromptAnalyzer(analysis_files)

    # Generate report
    report_path = pm.cross_analysis_report_path(output_name)
    analyzer.generate_report(report_path)

    # Generate visualizations
    heatmap_path = pm.cross_analysis_heatmap_path(output_name)
    analyzer.visualize_overlap(heatmap_path)

    network_path = pm.cross_analysis_network_path(output_name)
    ...
    consensus_path = pm.cross_analysis_consensus_path(output_name)
    with open(consensus_path, 'w') as f:
        json.dump(consensus, f, indent=2, default=str)
    print(f"[OK] Consensus supernodes saved to: {consensus_path.name}")

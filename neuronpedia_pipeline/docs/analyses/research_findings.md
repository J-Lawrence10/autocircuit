...
- Features now sorted by activation in output (highest first)
...
**Implementation Detail** (Fixed in latest version):
- Initial implementation: Features stored in arbitrary set() iteration order
- Fix: Added sorting by activation before saving to JSON
- Result: Analysis output now shows features in descending activation order for easier interpretation
